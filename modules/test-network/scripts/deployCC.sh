...
elif [ "$CC_SRC_LANGUAGE" = "scala" ]; then
  CC_RUNTIME_LANGUAGE=java
  pushd $CC_SRC_PATH
  cd ../..
  sbt "project contract" clean assembly
  popd
  CC_SRC_PATH=$CC_SRC_PATH/target/scala-2.12
...
packageChaincode() {
  set -x
  peer lifecycle chaincode package ${CC_NAME} --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ${CC_NAME}_${CC_VERSION} >&log.txt
  res=$?
  PACKAGE_ID=$(peer lifecycle chaincode calculatepackageid ${CC_NAME})
  { set +x; } 2>/dev/null
  cat log.txt
